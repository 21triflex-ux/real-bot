...
# Personal blackjack stats
@bot.command()
async def bjstats(ctx):
    uid = str(ctx.author.id)
    ensure_user(uid)
    s = user_data[uid]["stats"]
    e = discord.Embed(title=f"📊 {ctx.author.name}'s Blackjack Stats", color=discord.Color.green())
    ...
    win_percent = (s["blackjack_wins"]/s["total_blackjack_games"]*100) if s["total_blackjack_games"]>0 else 0
    e.add_field(name="📈 Win %", value=f"{win_percent:.2f}%")
    e.add_field(name="💵 CP Earned", value=s.get("cp_earned",0))
    e.add_field(name="💸 CP Lost", value=s.get("cp_lost",0))
    await ctx.send(embed=e)
...
class BlackjackView(discord.ui.View):
    ...
    async def split(self, i, b):
        ...
        self.pdata["hand"] = h1
        sid = self.pid + "_split"
        self.game["players"][sid] = {"hand": h2, "bet": self.pdata["bet"], "finished": False, "outcome": None}
        idx = self.game["turn_order"].index(self.pid)
        self.game["turn_order"].insert(idx+1, sid)
        await i.response.send_message("Split")
        self.stop()
...
async def bjstart(ctx):
    ...
    for uid in game["turn_order"]:
        ...
        if score == 21 and len(p["hand"])==2:
            ...
            p["finished"] = True
            p["outcome"] = ("blackjack", win)
            await ctx.send(f"🃏 {pl.mention} BLACKJACK! You win {win} CP")
            continue

        while not p["finished"]:
            v = BlackjackView(ctx, pid, p, game)
            await ctx.send(f"{pl.mention} {format_hand(p['hand'])} ({calculate_score(p['hand'])})", view=v)
            await v.wait()
    ...
    while dealer_score < 17:
        ...
        dealer_score = calculate_score(dealer)
    await ctx.send(f"Dealer {format_hand(dealer)} ({dealer_score})")
    ...
    for uid, p in game["players"].items():
        ...
        pl = await bot.fetch_user(int(pid))
        if p.get("outcome"):
            continue  # already handled blackjack or bust
